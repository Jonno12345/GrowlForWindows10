...
namespace GrowlWindows10Notifier
{
    ...
    public partial class SampleSettingsPanel : SettingsPanelBase
    {
        ...
        {
            string filename = GetFileFromSetting(Environment.ExpandEnvironmentVariables(@"%LOCALAPPDATA%\Growl\2.0.0.0\Displays\Windows 10 Notification\WindowsNotification.exe"));
        ...
        void openFileDialog1_FileOk(object sender, CancelEventArgs e)
        {
            ...
            // persist the setting
            this.SaveSetting(Windows10Notifier.SETTING_FILE_NAME, this.textBox1.Text);
        }
        ...
        private string GetFileFromSetting(string defaultFileName)
        {
            ...
            if (settings != null && settings.ContainsKey(Windows10Notifier.SETTING_FILE_NAME))
            {
                try
                {
                    object val = settings[Windows10Notifier.SETTING_FILE_NAME];
                    if (val is string)
                    {
                        filename = (string)val;
                        isDefault = false;
            ...
            if (isDefault) SaveSetting(Windows10Notifier.SETTING_FILE_NAME, filename);
